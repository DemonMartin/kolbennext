...
export default function PlayComponent({
    ...
    const [totalAudioLevel, setTotalAudioLevel] = useState([0]);
        ...
        if (!started) {
            ...
            intervalIdRef.current = setInterval(() => {
                ...
                if (fluidLevelRef.current < 1) {
                    ...
                    if (audioLevelRef.current > 11 && fluidLevelRef.current < 1) {
                        const addFluidLevel = (audioLevelRef.current / 40000) + 0.006;
                        ...
                        setFluidLevel(Math.max(fluidLevelRef.current - 0.005, 0)); // Decrease the step size
                    }
                } else {
                    stopGame();
                }
    ...
    return (
        <div className={`relative p-4 md:p-6 rounded-3xl shadow-xl transition-shadow duration-300 border border-gray-200 bg-white bg-opacity-25 text-white ${started ? "shadow-2xl" : ""}`}>
            ...
            <ImageComponent src={imgSrc} id={id} fluidLevel={fluidLevelRef.current} audioLevel={audioLevel} running={started} />
            {
                ...
                <span>Average Audio Level: {totalAudioLevel.reduce((a, b) => a + b, 0) / totalAudioLevel.length}</span>
            }
